...
const KnowledgeTree = (() => {
  ...
  function drawCrystals() {
    for (const m of mainCrystals) {
      ...
      m.hoverEase = lerp(m.hoverEase, (hovered === m || (selected && selected._mainRef === m)) ? 1 : 0, reduced ? 1 : 0.12);
  ...
  function allClickable() {
    const out = [];
    for (const m of mainCrystals) { if (m.appear > 0.4) out.push(m); for (const sk of m.skills) if (sk.reveal > 0.5) { sk._mainRef = m; out.push(sk); } }
    return out;
  }
