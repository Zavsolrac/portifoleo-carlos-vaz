...
const Crystals = {
    ...
    material.onBeforeCompile = (shader) => {
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace(
          ...
          // Mix it all into emissive output
          totalEmissiveRadiance += emissive * caustic * uCausticStrength * (1.0 + uHover * 0.8);
          totalEmissiveRadiance += emissive * fresnel * uFresnelStrength * (1.0 + uHover * 1.2);
          totalEmissiveRadiance += emissive * spark * 0.6;

          // Modulate diffuse by noise (interior imperfections darken/lighten)
          diffuseColor.rgb *= mix(0.82, 1.20, n);
          // Hover boosts overall brightness slightly
          diffuseColor.rgb *= (1.0 + uHover * 0.15);
          `
        );
  ...
  open(project, sourceEl) {
    ...
    // #region agent log
    (function debugVaultMotion() {
      try {
        var EP = 'http://127.0.0.1:7279/ingest/89c13b11-4c60-49a0-81e3-64782c804124';
        var send = function (h, msg, data) {
          fetch(EP, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Debug-Session-Id': 'bc6917' }, body: JSON.stringify({ sessionId: 'bc6917', hypothesisId: h, location: 'crystals.js:open', message: msg, data: data, timestamp: Date.now() }) }).catch(function () {});
        };
        var card = document.querySelector('.crystal-vault .relic-card');
        var bd = document.getElementById('crystal-vault-backdrop');
        var section = document.getElementById('memories');
        var bf = bd ? (getComputedStyle(bd).backdropFilter || getComputedStyle(bd).webkitBackdropFilter || 'none') : 'none';
        send('B', 'vault-open-init', { runId: 'post-fix-v2', backdropFilter: bf, sceneRunning: !!(window.Crystals._three && window.Crystals._three.running), vaultGemRunning: !!(window.Crystals._vaultThree && window.Crystals._vaultThree.running), scenePausedForVault: !!document.querySelector('.crystal-vault.is-open'), reduced: !!window.Crystals._reducedMotion });
        var tops = [], n = 0, lastMx = null, lastMy = null, bgMoves = 0, tf = [];
        var step = function () {
          var r = card ? card.getBoundingClientRect() : null;
          var cs = card ? getComputedStyle(card).transform : '';
          var mx = section ? section.style.getPropertyValue('--mx') : '', my = section ? section.style.getPropertyValue('--my') : '';
          if (mx !== lastMx || my !== lastMy) { bgMoves++; lastMx = mx; lastMy = my; }
          tops.push(r ? +r.top.toFixed(2) : null);
          if (n === 0 || n === 30 || n === 55 || n === 75 || n === 119) tf.push({ n: n, t: (cs || '').slice(0, 46) });
          n++;
          if (n < 120) { requestAnimationFrame(step); }
          else {
            var valid = tops.filter(function (x) { return x != null; });
            var maxJump = 0, dirChanges = 0, prevTop = null, prevDelta = 0, jumpsOver1 = 0;
            for (var i = 0; i < tops.length; i++) {
              var s = tops[i]; if (s == null) continue;
              if (prevTop != null) {
                var d = s - prevTop;
                if (Math.abs(d) > maxJump) maxJump = Math.abs(d);
                if (Math.abs(d) > 1) jumpsOver1++;
                if (prevDelta !== 0 && Math.sign(d) !== Math.sign(prevDelta) && Math.abs(d) > 0.3) dirChanges++;
                prevDelta = d;
              }
              prevTop = s;
            }
            send('A/C/D', 'vault-card-motion', { runId: 'post-fix-v2', frames: tops.length, topMin: valid.length ? Math.min.apply(null, valid) : null, topMax: valid.length ? Math.max.apply(null, valid) : null, maxFrameJump: +maxJump.toFixed(2), jumpsOver1px: jumpsOver1, dirChanges: dirChanges, bgParallaxMoves: bgMoves, transforms: tf });
          }
        };
