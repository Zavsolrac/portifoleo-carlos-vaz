...
const Merlin = {
  ...
  showScroll(payload) {
    ...
    // Reading window: piso confortável (≥ 8 s), teto generoso (≤ 18 s)
    // e ritmo de ~65 ms/caractere. Antes era 5.5–12 s a 55 ms/char,
    // o que cortava quotes longas antes do tempo. O usuário relatou
    // que algumas mensagens sumiam antes da leitura completa.
    const totalLen = (title + body).length;
    const readMs = Math.min(18000, Math.max(8000, Math.round(totalLen * 65)));
  ...
  brainContextBlock() {
    ...
    lines.push("");
    lines.push("SERVICES (Arcane Contracts, paid):");
    lines.push("• Landing Page Arcana (Common): €350–700 / R$2.200–4.400.");
    lines.push("• Site Institucional (Uncommon): €700–1.500 / R$4.400–9.400.");
    lines.push("• Portfólio Profissional (Rare): €500–1.200 / R$3.100–7.500.");
    lines.push("• Automação Arcana e IA (Epic): €300+ / R$1.900+.");
    lines.push("• Projeto Sob Medida (Legendary): custom quote.");
  ...
  startIdleTicker() {
    this._idleTimer = setInterval(() => {
      if (!this.el || this._scrollVisible) return;
      const quietMs = Date.now() - this._lastInteraction;
      if (quietMs < 25000) return;
      this.castSpell({ count: 6 });
      this.speakIdle();
      this._lastInteraction = Date.now();   // give 25s before next prompt
    }, 6000);
  },
