...
const SkillTree = (() => {
  ...
  function activateNode(node) {
    ...
    // 3-skill milestone: counts user-activated nodes (mastered minus the
    // pre-active core + keystones). Fires on the 3rd activation so the
    // "click on perfil_carlos" nudge appears early in the journey.
    // Cleared by the reset button.
    userActivationCount += 1;
    const preActive = 1 + CLUSTERS.length; // core + keystones
    const userActivated = Math.max(userActivationCount, mastered.size - preActive);
    if (
      !milestoneFired &&
      userActivated >= MILESTONE_TARGET &&
      localStorage.getItem(MILESTONE_KEY) !== "1"
    ) {
      milestoneFired = true;
      try { localStorage.setItem(MILESTONE_KEY, "1"); } catch { /* ignore */ }
      window.dispatchEvent(new CustomEvent("cv-milestone-8skills", {
        detail: { count: userActivated, node },
      }));
    }
    return true;
  }
  ...
  function drawBackground() {
    ...
    const bg = ctxBg.createRadialGradient(w * 0.5, h * 0.45, 0, w * 0.5, h * 0.5, Math.max(w, h) * 0.7);
    bg.addColorStop(0, "#0c1420");
    bg.addColorStop(0.45, "#080f18");
    ...
    stars.forEach((s) => {
      const sp = worldToScreen(s.x, s.y + scrollY);
  ...
  function drawEdges() {
    ...
    edges.forEach((e) => {
      ...
      if (both) {
        ...
        if (light) {
          ...
          ctxFg.shadowColor = "rgba(184, 138, 59, 0.45)";
          ctxFg.shadowBlur = 5 * e.flow;
        } else {
          ...
          ctxFg.shadowColor = "rgba(225, 190, 110, 0.85)";
          ctxFg.shadowBlur = 10 * e.flow;
        }
      } else {
        // Conexões inativas
        ctxFg.strokeStyle = light
    ...
    edges.forEach((e) => {
      ...
      ctxFg.shadowColor = `rgba(${tint.r},${tint.g},${tint.b},0.85)`;
      ctxFg.shadowBlur = light ? 9 : 14;
      ctxFg.stroke();
  ...
  function drawNodes() {
    ...
    const sorted = [...nodes].sort((a, b) => {
      const za = a.active ? 1 : 0;
      const zb = b.active ? 1 : 0;
      return za - zb;
    });

    for (const n of sorted) {
      const p = worldToScreen(n.x, n.y);
      const screenR = n.r * cam.zoom;
      if (screenR < 1.2) continue;
      if (p.x < -80 || p.x > canvasFg.clientWidth + 80 || p.y < -80 || p.y > canvasFg.clientHeight + 80) continue;
      ...
      if (showLabels && n.notable && screenR > 4) {
        ...
        if (light) {
          // Tinta marrom escura sobre pergaminho — após o multiply
          // do canvas continua nítida.
  ...
  function recenter() {
    targetCam.x = CX;
    targetCam.y = CY + window.scrollY * 0.08;
    targetCam.zoom = 0.7;
  }
